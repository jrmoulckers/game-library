...
function renderRoots(tbody) {
  ...
  if (rootRows.length === 0) {
    tbody.appendChild(
      el("tr", {}, [el("td", { colspan: "6", text: "No roots configured yet. Use \u201cAdd root\u201d to add the first one." })]),
    );
    return;
  }
  rootRows.forEach((row, index) => {
    ...
    const removeButton = el("button", {
      ...
      onclick: () => {
        ...
        const nextTarget = nextRow?.querySelector("input, button") || document.getElementById("setup-add-root");
        nextTarget?.focus();
      },
    });
    tbody.appendChild(
...
export async function init(root, { api, announceStatus, announceError }) {
  ...
  form.addEventListener("submit", async (event) => {
    ...
    const config = {
      ...
      roots: rootRows.map((r) => ({ id: r.id, kind: r.kind, path: r.path, system: r.system || undefined })),
    ...
    try {
      ...
      if (err.code === "config_conflict") {
        statusEl.textContent = "Save conflict: the configuration changed elsewhere. Reloading the current version — review it and save again.";
        announceError("Configuration save conflict: reloaded the current version. Review and save again.");
        try {
          await loadConfig();
        } catch (reloadErr) {
          announceError(`Failed to reload the active configuration after a conflict: ${reloadErr.message}`);
        }
      } else {
