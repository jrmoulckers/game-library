  ...
  const button = el(
    ...
    [
      ...
      el('span', { class: 'platform-card__body' }, [
        ...
        el('span', {
          text: `${platform.gameCount} ${platform.gameCount === 1 ? 'game' : 'games'}`,
        }),
        el('span', {
...
function renderLibrary() {
  ...
  if (catalog.needsAttention) {
    summary.append(
      ` ${catalog.needsAttention} ${catalog.needsAttention === 1 ? 'item needs' : 'items need'} attention.`,
    );
  }
...
function visibleGames() {
  ...
  games.sort((a, b) => {
    if (sort === 'coverage')
      return a.missingRoles.length - b.missingRoles.length || a.title.localeCompare(b.title);
...
function representativeAsset(game) {
  return (
    game.assets.find((asset) => ['portrait', 'cover', 'grid', 'miximage'].includes(asset.role)) ||
    game.assets[0]
  );
}
  ...
  return el(
    ...
    [
      ...
      el('span', {
        ...
        text: game.missingRoles.length
          ? `${game.assets.length} artwork ${game.assets.length === 1 ? 'role' : 'roles'}`
          : 'Artwork complete',
      }),
    ],
  );
...
function openPreview(asset, title) {
  ...
  figure.appendChild(
    el('figcaption', {
      text: `${title} · ${asset.role} · ${asset.width || '?'}x${asset.height || '?'}`,
    }),
  );
  dialog.showModal();
}
...
function openGame(game) {
  ...
  clear(identities);
  for (const [namespace, value] of Object.entries(game.identities || {}))
    identities.appendChild(chip(`${namespace}: ${value}`));
  ...
  if (game.missingRoles.length) {
    ...
    missing.appendChild(
      el(
        'div',
        { class: 'chip-row' },
        game.missingRoles.map((role) => chip(role, 'chip--missing')),
      ),
    );
  }
...
function renderGameCoverage(game) {
  ...
  for (const profile of entry.profiles) {
    list.appendChild(
      el('li', { class: profile.covered ? 'coverage-row' : 'coverage-row coverage-row--gap' }, [
        el(
          'button',
          {
            type: 'button',
            class: 'coverage-row__name',
        ...
        el(
          'div',
          { class: 'chip-row coverage-row__devices' },
...
function profileCard(profile) {
  ...
  return el(
    'article',
    { class: profile.empty ? 'profile-card profile-card--empty' : 'profile-card' },
    [
      el(
        'button',
        {
          type: 'button',
          class: 'profile-card__open',
          onclick: () => openProfile(profile.key),
...
async function renderProfiles(api) {
  ...
  for (const [platformId, profiles] of byPlatform) {
    ...
    host.appendChild(
      el('section', { class: 'platform-group' }, [
...
function renderLiveSurfaces() {
  ...
  if (!coverage.liveSurfaces.length) {
    list.appendChild(
      el('p', {
        class: 'empty-state',
        text: 'No live frontend folders are configured on this machine.',
      }),
    );
  }
}
...
async function assignArtwork(api, artworkSet, key) {
  ...
  try {
    ...
    const target = doc.profiles.find(
      (profile) => `${profile.platform}/${slugify(profile.name)}` === key,
    );
    if (!target) {
    ...
    window.announceStatus(
      `"${artworkSet}" is now the artwork for ${target.platform} · ${target.name}. No files were changed.`,
    );
  } catch (err) {
...
function renderProfileGames() {
  ...
  const games = activeProfile.games.filter(
    (game) => !term || game.title.toLowerCase().includes(term),
  );
  ...
  for (const game of games) {
    ...
    list.appendChild(
      el('article', { class: 'coverage-card' }, [
        el(
          'button',
          {
            type: 'button',
            class: 'coverage-card__open',
            onclick: () => {
              if (full) openGame(full);
            },
            'aria-label': `Open ${game.title}`,
          },
          [
            ...
            el(
              'div',
              { class: 'chip-row' },
              game.roles.map((role) => chip(role, 'chip--role')),
            ),
            el('p', {
              class: 'context-line',
              text: `${game.assetCount} ${game.assetCount === 1 ? 'file' : 'files'}`,
            }),
          ],
        ),
      ]),
...
async function renderSources(api) {
  ...
  const detectedByPath = new Map(
    (detected.sources || []).map((item) => [pathKey(item.path), item]),
  );
  const roots = config.config?.roots || [];
  const items = [...(detected.sources || []), ...(detected.supported || [])];
  const diagnosticsFor = (kind) =>
    (detected.metadataDiagnostics || []).filter((diagnostic) => {
  ...
  if (!items.length) {
    grid.appendChild(
      el('p', {
        class: 'empty-state',
        text: 'No artwork sources were detected. Add a folder manually under Source folders.',
      }),
    );
    return detected;
  }
      ...
      el('article', { class: 'source-card' }, [
        el('div', {}, [
          ...
          el('p', {
            text:
              source.message ||
              (source.itemCount === null ? 'Configured source' : `${source.itemCount} items found`),
          }),
        ...
        el('div', { class: 'source-actions' }, [
          chip(
            ...
            connected
              ? 'chip--connected'
              : unavailable || source.status === 'not-on-this-device'
                ? 'chip--neutral'
                : '',
          ),
          source.configured
...
async function setupOnboarding(api, announceStatus, announceError) {
  ...
  if (!(detected.sources || []).length) {
    list.appendChild(
      el('p', {
        class: 'empty-state',
        text: 'No common artwork locations were found. Add a folder manually below.',
      }),
    );
  }
  document.getElementById('detected-sources-form').addEventListener('submit', async (event) => {
    ...
    const roots = (detected.sources || [])
      .filter((source) => selected.has(source.id))
      .map(({ id, kind, path, system }) => ({ id, kind, path, system }));
    if (!roots.length) {
    ...
    try {
      await api.put('/api/config', {
        ...
        config: {
          version: 1,
          roots,
          policy: { version: 1, default: 'tracked-external', rules: [] },
        },
      });
...
export async function init({ api, announceStatus, announceError }) {
  ...
  document
    ...
    .addEventListener('click', () => openPlatform(activePlatform));
  document
    .getElementById('profile-back')
    .addEventListener('click', () => showView('profile-library'));
  ...
  document
    .getElementById('profile-game-filters')
  ...
  document
    ...
    .addEventListener('click', () => document.getElementById('artwork-preview').close());
