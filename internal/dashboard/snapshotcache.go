...
func (c *snapshotCache) getOrLoad(cfg model.Config, reportPath string) (review.Snapshot, error) {
	...
	if err != nil {
		return review.Snapshot{}, fmt.Errorf("digest snapshot configuration: %w", err)
	}
	c.mu.Lock()
	if c.configDigest == "" {
		c.configDigest = digest
	}
	if c.configDigest != digest {
		c.mu.Unlock()
		return review.Snapshot{}, fmt.Errorf("snapshot configuration changed")
	}
	if c.hasValue {
		value, cachedErr := c.value, c.err
		c.mu.Unlock()
...
func (c *snapshotCache) current() (review.Snapshot, bool, error) {
	c.mu.Lock()
	defer c.mu.Unlock()
	return c.value, c.hasValue, c.err
}
...
func (c *snapshotCache) invalidateFor(cfg model.Config) error {
	digest, err := manifest.Digest(cfg)
	if err != nil {
		return err
	}
...
func (c *snapshotCache) currentRevision() uint64 {
	c.mu.Lock()
	defer c.mu.Unlock()
	return c.revision
}

func (c *snapshotCache) storeIfRevision(snapshot review.Snapshot, revision uint64) bool {
	c.mu.Lock()
	defer c.mu.Unlock()
...
func (c *snapshotCache) reload(cfg model.Config, reportPath string) (review.Snapshot, error) {
	digest, digestErr := manifest.Digest(cfg)
	if digestErr != nil {
		return review.Snapshot{}, fmt.Errorf("digest snapshot configuration: %w", digestErr)
	}
	c.mu.Lock()
	if c.configDigest == "" {
		c.configDigest = digest
...
func (c *snapshotCache) store(snapshot review.Snapshot) {
	c.mu.Lock()
	defer c.mu.Unlock()
	c.value = snapshot
	c.err = nil
